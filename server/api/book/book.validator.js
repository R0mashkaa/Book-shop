...
const { BOOKNAME_REGEXP, AUTHOR_REGEXP } = require('../../configs/regexp.enum');

const joiValidatorSchema = Joi.object({

    bookName: Joi.string().regex(BOOKNAME_REGEXP).required().min(4).max(32).error(new Error('Book name is not valid')),
